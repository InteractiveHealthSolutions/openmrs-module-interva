DROP PROCEDURE interva_inputcsv_query;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `interva_inputcsv_query`()
BEGIN
...
-- CSV and OBS Concept mapping are defined in table interva_input_mapping with order required
	DECLARE cObs CURSOR FOR 
			SELECT interva_variable, is_observation, c.concept_id, cdt.name, interva_index, accept_condition FROM interva_input_mapping i 
			LEFT JOIN concept c ON c.concept_id=i.concept_id LEFT JOIN concept_datatype cdt ON c.datatype_id=cdt.concept_datatype_id 
			ORDER BY interva_index;
...
SET @interva_temp_table = 'interva_temp_table';
...
-- InterVA is used to find COD on Verbal Autopsy forms ONLY 
...
-- JOIN parts of query for temp table for OBS of specified concept (from interva_input_mapping table)
SET @qJoinsObs = '';
...
-- Current JOIN of OBS in loop for temp table (this is represented by each row of interva_input_mapping for which observation=true)
SET @currentRow = 1;
read_loop: LOOP
...
IF done THEN
  LEAVE read_loop;
ELSE 
-- FINAL columns in CSV would be represented by variable_name in each row of interva_input_mapping table
...
-- Each OBS of interva_input_mapping would be JOINed as o_INTERVA_MAPPING_VARIABLE_NAME, 
-- so wherever it finds OBS in interva accept condition it would replace it with the table alias in query being created
-- EX: OBS.value_numeric < 3 in accept condition for interva mapping ACUTE would be converted to 
...
-- AND o_ACUTE.concept_id=interva_input_mapping.MAPPED_CONCEPT_FOR_VARABLE_IN_CSV 
-- AND o_ACUTE.obs_id =(MAX OBS_ID FOR SELECTED CRITERIA to ensure smoothness of process in case of duplications)
		...
		END IF;
-- EX: P.gender = F in accept condition for interva mapping GENDER would be converted to 
...
-- if 1-nth append person_id and encounter_id in temptable so that final great table could be joined with different set of obs
			...
			SET @tmpTableName=CONCAT(@interva_temp_table, @tempTables);
...
-- if still unprocessed variables/rows in interva_input_mapping increment temp table for next
...
SET @finalQueryString = CONCAT("SELECT ", @finalColumns, " FROM ", @interva_temp_table,@loopindex);
...
-- append JOIN next temp table on ID and EID
		SET @finalQueryString = CONCAT(@finalQueryString, " JOIN ", @interva_temp_table,@loopindex, " USING(ID, EID) ");
	END IF;
  END LOOP;
