...
function showIvaMappingDetails(objid){
	var detailstr = "<html><head><title>InterVA OUTPUT Mapping ("+objid+")</title></head>"+
					"<style>"+
					...
					"</style><body>"+
					"<div class='heading'>InterVA OUTPUT Mapping</div>"+
					"<table class='datatableiva'>";
	<c:forEach items="${outputmappings}" var="item" varStatus="loop"> 
    	if('${item.intervaOutputMappingId}' == (''+objid)){
    		detailstr += 
    			"<tr><th>Mapping ID </th><td>${item.intervaOutputMappingId}</td></tr>"+
    			"<tr><th>Variable name</th><td>${item.intervaOutputVariable}</td></tr>"+
    		   	"<tr><th>Index</th><td>${item.intervaOutputIndex}</td></tr>"+
...
<div class="heading">InterVA Mappings for OUTPUT FILE</div>
<div id="detailsDiv" class=""></div>
<table class="datatableiva">
  <tr>
   <th></th>
   <th>Mapping ID</th>
  ...
  <c:forEach var="oml" items="${outputmappings}">
      <tr>
      	<td><a onclick="showIvaMappingDetails('${oml.intervaOutputMappingId}');">details</a></td>
        <td>${oml.intervaOutputMappingId}</td>
        <td>${oml.intervaOutputVariable}</td>
        <td>${oml.intervaOutputIndex}</td>
