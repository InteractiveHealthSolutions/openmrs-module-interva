...
function showIvaMappingDetails(objid){
	var detailstr = "<html><head><title>InterVA INPUT Mapping ("+objid+")</title></head>"+
					"<style>"+
					...
					"</style><body>"+
					"<div class='heading'>InterVA INPUT Mapping</div>"+
					"<table class='datatableiva'>";
	<c:forEach items="${inputmappings}" var="item" varStatus="loop"> 
    	if('${item.intervaMappingId}' == (''+objid)){
    		detailstr += 
    			"<tr><th>Mapping ID </th><td>${item.intervaMappingId}</td></tr>"+
    			"<tr><th>Variable name</th><td>${item.intervaVariable}</td></tr>"+
    		   	"<tr><th>Index</th><td>${item.intervaIndex}</td></tr>"+
...
<div class="heading">InterVA Mappings for INPUT CSV file</div>
...
<table class="datatableiva">
  ...
  <c:forEach var="iml" items="${inputmappings}">
      <tr>
      	<td><a onclick="showIvaMappingDetails('${iml.intervaMappingId}');">details</a></td>
        <td>${iml.intervaMappingId}</td>
        <td>${iml.intervaVariable}</td>
        <td>${iml.intervaIndex}</td>
